...
-- Modificación de la tabla `notas` para agregar la columna `asignatura_id`
ALTER TABLE `claserepaso`.`notas`
ADD COLUMN `asignatura_id` BIGINT NULL DEFAULT NULL,
ADD INDEX `FK_notas_asignatura` (`asignatura_id` ASC),
ADD CONSTRAINT `FK_notas_asignatura`
  FOREIGN KEY (`asignatura_id`)
  REFERENCES `claserepaso`.`asignaturas` (`asignatura_id`);
  
  -- Ejemplo: Asigna `asignatura_id` a las notas existentes, suponiendo que la asignatura con id 1 sea "Servidor"
UPDATE `claserepaso`.`notas` SET `asignatura_id` = 1 WHERE `nombre_trabajo` = "Api servidor";

-- Incorporación de nuevos registros para la tabla `notas`
INSERT INTO notas VALUES (6, 1, 0, "micro servidor",1);
INSERT INTO notas VALUES (8, 2, 0, "micro servidor",1);
INSERT INTO notas VALUES (5, 2, 0, "web",2);
INSERT INTO notas VALUES (4, 1, 0, "diseño",2);
INSERT INTO notas VALUES (9, 1, 0, "web",2);
